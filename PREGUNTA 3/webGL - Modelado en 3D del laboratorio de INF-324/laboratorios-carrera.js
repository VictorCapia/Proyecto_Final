...
function init() {
    ...
    //puerta 1 central izquierdo 1
    //objetocub(-0.65,-0.3,-0.6,-0.3,-0.6,0.6,-0.65,0.6,0.01,-0.2,0.2,0x873600,0xBA4A00);

    //puerta 2 central derecho
    //objetocub(0.0,-0.3,0.05,-0.3,0.05,0.6,0.0,0.6,0.01,0.0,0.4,0x873600,0xBA4A00);

     //pared lateral izquierdo
    objetocub(-2.4, -0.3, -2.3, -0.3, -2.3, 0.9, -2.4, 0.9, 0.01, -3.5, 4.5, 0xF9E79F, 0xF9E79F);
    
    //pared lateral derecho
    objetocub(1.7, -0.3, 1.6, -0.3, 1.6, 0.9, 1.7, 0.9, 0.01, -3.1, 4.5, 0xF9E79F, 0xF9E79F);
    
    //pared de arriba de la puerta principal
    objetocub(1.7, 0.6, 1.6, 0.6, 1.6, 0.9, 1.7, 0.9, 0.01, -3.6, -2.6, 0xF9E79F, 0xF9E79F);
    
    //cubo reemplazando a un curso
    //objetocub(0.1,-0.3,2.6,-0.3,2.6,0.9,0.1,0.9,0.02,-2.6,-1.0,0x7FB3D5,0xF9E79F);
    
    //pared cubico del fondo lado derecho
    objetocub(-1.0, -0.3, 1.7, -0.3, 1.7, 0.9, -1.0, 0.9, 0.02, -3.9, -3.58, 0x7FB3D5,0xF9E79F);

    //pared cubico del fondo lado izquierdo
    objetocub(-2.4, -0.3, -1.0, -0.3, -1.0, 0.9, -2.4, 0.9, 0.01, -3.9, -3.5, 0x7FB3D5, 0xFAFFD7);

    //pared 3 de pizarra lado izquierdo
    //objetocub(-2.4,-0.3,-0.6,-0.3,-0.6,0.9,-2.4,0.9,0.02,-0.22,-0.2,0xACE1D1,0xF9E79F);
    
    //pared 4 de pizarra lado izquierdo
    //objetocub(-2.4,-0.3,-0.6,-0.3,-0.6,0.9,-2.4,0.9,0.02,-2.9,-2.8,0xF9E79F,0xF9E79F);
    
    //puerta abierta principal
    objetocub(1.7, -0.4, 1.3, -0.4, 1.3, 0.6, 1.7, 0.6, 0.01, -3.5, -3.5, 0x873600, 0xBA4A00);
    
    //pizarra principal del fondo
    objetocub(-2, 0.05, -1, 0.05, -1, 0.7, -2, 0.7, 0.02, -3.39, -3.4, 0xAEB6BF, 0xF8F9F9);
    objetocub(-2, 0.72, -1, 0.72, -1, 0.8, -2, 0.8, 0.02, -3.39, -3.4, 0xAEB6BF, 0xF8F9F9);
    
    //pizarra secundario del fondo
    objetocub(-0.5, 0.05, 1.2, 0.05, 1.2, 0.7, -0.5, 0.7, 0.02, -3.39, -3.4, 0xAEB6BF, 0xF8F9F9);
    objetocub(-0.5, 0.72, 1.2, 0.72, 1.2, 0.8, -0.5, 0.8, 0.02, -3.39, -3.4, 0xAEB6BF, 0xF8F9F9);

    //piso
    objetocub(-2.4, -0.55, 1.7, -0.55, 1.7, -0.31, -2.4, -0.31, 0.05, -4.0, 4.5, 0xAAB7B8,0xAED6F1);
    
    //plataforma 1 lado derecho
    //objetocub(0.4,-0.3,2,-0.3,1.9,-0.2,0.5,-0.2, 0.02,2.52,2.8,0x873600,0xBA4A00);
    
    //plataforma 1 lado izquierdo
    //objetocub(-2,-0.3,-0.9,-0.3,-1,-0.2,-1.9,-0.2, 0.02,2.52,2.8,0x873600,0xBA4A00);
    
    //plataforma 2 lado derecho
    //objetocub(0.5,-0.3,2,-0.3,1.9,-0.2,0.5,-0.2, 0.02,-0.21,0.07,0x873600,0xBA4A00);
    
    //plataforma 2 lado izquierdo
    //objetocub(-2,-0.3,-0.9,-0.3,-1,-0.2,-1.9,-0.2, 0.02,-0.21,0.07,0x873600,0xBA4A00);
    
    //plataforma principal
    objetocub(-2, -0.3, -0.9, -0.3, -1, -0.2, -1.9, -0.2, 0.02, -3, -3.4, 0x873600, 0xBA4A00);

    //lado izquierdo
    //(5 columnas, 6 filas)
    PCs(-2, 0.0, -1.8, 0.0, -1.8, 0.15, -2, 0.15, -1.5, 4, 6);
    //PCs(-1.0,0.0, -0.8,0.0, -0.8,0.15, -1.0,0.15,3.2, 2,2);
    
    //PCs del salon 4 columna izquierda de 2 maguinas
    //PCs(-2.3, 0.0, -2.1, 0.0, -2.1, 0.15, -2.3, 0.15, -2.2, 2,4);

    //PCs del salon 4 columna derecha de 2 maguinas
    //PCs(-1.3, 0.0, -1.1, 0.0, -1.1, 0.15, -1.3, 0.15, -2.4, 2,4);
   
    //lado derecho
    //(5 columnas, 4 filas)
    PCs(0.0, 0.0, 0.2, 0.0, 0.2, 0.17, 0.0, 0.17, -1.5, 5, 6);
    //PCs(1.5, 0.0, 1.7, 0.0, 1.7, 0.15, 1.5, 0.15, 3.5, 2,2);
    
    //PC individual al extremo derecho
    //PCs(2.2, 0.0, 2.4, 0.0, 2.4, 0.15, 2.2, 0.15, 3.2, 1,1);

    PCss(-2.7, 0.0, -2.5, 0.0, -2.5, 0.15, -2.7, 0.15, 2,1); 
    
    //mesa y silla
    mesa(0.5, 0.0, 1.5, 0.0, 1, 1);
